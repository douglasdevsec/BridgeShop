...
export function createConfigClient(isAdmin = false) {
  ...
  // Exclude @bridgeshop/BridgeShop core and extensions in node_modules from managed paths
  ...
  const managedPathsPattern = nodeModuleExtensions
    ? `^(.+?[\\\\/]node_modules[\\\\/](?!(@BridgeShop[\\\\/]BridgeShop|${nodeModuleExtensions}))(@.+?[\\\\/])?.+?)[\\\\/]`
    : `^(.+?[\\\\/]node_modules[\\\\/](?!(@BridgeShop[\\\\/]BridgeShop))(@.+?[\\\\/])?.+?)[\\\\/]`;
