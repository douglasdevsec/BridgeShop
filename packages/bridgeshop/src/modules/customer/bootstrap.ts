...
export default () => {
   ...
   * @param {*} callback
   */
  (request as BridgeShopRequest).loginCustomerWithEmail = async function login(
    email,
    password,
    callback
  ) {
  ...
  (request as BridgeShopRequest).logoutCustomer = function logout(callback) {
  ...
  (request as BridgeShopRequest).isCustomerLoggedIn =
    function isCustomerLoggedIn() {
      return !!this.session?.customerID;
    };

  (request as BridgeShopRequest).getCurrentCustomer =
