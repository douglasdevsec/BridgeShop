...
export async function buildEntry(routes, clientOnly = false) {
  ...
  await Promise.all(
    routes.map(async (route) => {
      ...
      import { Area } from '@bridgeshop/BridgeShop/components/common';
      import {${
        route.isAdmin ? 'HydrateAdmin' : 'HydrateFrontStore'
      }} from '@bridgeshop/BridgeShop/components/common';
      `;
      areas['*'] = areas['*'] || {};
      ...
      if (!clientOnly) {
        ...
        contentServer += '\r\n';
        contentServer += `import { Area } from '@bridgeshop/BridgeShop/components/common';`;
        contentServer += '\r\n';
        contentServer += `import { renderHtml } from '@bridgeshop/BridgeShop/components/common';\r\n`;
