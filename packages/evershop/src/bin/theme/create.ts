      ...
      compilerOptions: {
        ...
        paths: {
          '@components/*': (await isRealDirectory(
            path.join(process.cwd(), 'node_modules', '@BridgeShop', 'BridgeShop')
          ))
            ? [
                './src/components/*',
                '../../node_modules/@bridgeshop/BridgeShop/src/components/*'
              ]
            : ['./src/components/*', '../../packages/BridgeShop/src/components/*']
        }
      },
      include: ['src']
    };
