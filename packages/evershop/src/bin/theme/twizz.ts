...
async function scanModulesFrontStore(): Promise<string[]> {
  ...
  const modulesDir = (await isRealDirectory(BridgeShopDir))
    ? path.join(BridgeShopDir, 'src', 'modules')
    : path.join(process.cwd(), 'packages', 'BridgeShop', 'src', 'modules');
  let results: string[] = [];
  try {
...
async function getOverrideCandidates(): Promise<string[]> {
  // Check if a folder @bridgeshop/BridgeShop exists in the node_modules
  const BridgeShopDir = path.join(
    process.cwd(),
    'node_modules',
    '@BridgeShop',
  ...
  if (await isRealDirectory(BridgeShopDir)) {
    commonDir = path.join(BridgeShopDir, 'src', 'components', 'common');
    frontStoreDir = path.join(BridgeShopDir, 'src', 'components', 'frontStore');
  } else {
    commonDir = path.join(
      process.cwd(),
      'packages',
...
async function createOverrideFile() {
  ...
  const baseDir = (await isRealDirectory(BridgeShopDir))
    ? path.join(BridgeShopDir, 'src')
    : path.join(process.cwd(), 'packages', 'BridgeShop', 'src');
