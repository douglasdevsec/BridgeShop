...
export function createBaseConfig(isServer) {
  ...
  // Resolve aliases
  const alias = {
    '@bridgeshop/BridgeShop/components': path.resolve(__dirname, '../../components')
  };
  if (theme) {
  ...
  if (
    !isRealDirectorySync(
      path.resolve(CONSTANTS.ROOTPATH, 'node_modules', '@BridgeShop', 'BridgeShop')
    )
  ) {
    alias['@bridgeshop/BridgeShop'] = path.resolve(
      CONSTANTS.ROOTPATH,
      'packages',
      'BridgeShop',
      'dist'
